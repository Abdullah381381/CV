...
class QuestionsInterview extends StatefulWidget {
  const QuestionsInterview({super.key});

  @override
  State<QuestionsInterview> createState() => _QuestionsInterviewState();
}

class _QuestionsInterviewState extends State<QuestionsInterview> {
  ...
  void initState() {
    super.initState();
    databaseReference = FirebaseDatabase.instance.reference().child('QuestionsInterview');
    fetchData();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      floatingActionButton:user?.email=="mdiaa7788@gmail.com" ?FloatingActionButton(
        backgroundColor: primary2Color,
        tooltip: 'Increment',
        onPressed: () async{
          await  Navigator.push(
            context,
            MaterialPageRoute(builder: (ctx) =>  AddQuestionsInterview()),
          );
          setState(() {
      ...
      appBar: AppBar(
        title: const Text('Questions Interview'),
      ),
